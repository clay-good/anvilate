...
one symbol in one document, in 90 of 389 legend rows across the suite. Matching a symbol in the
working to its row in the legend is the thing a reviewer actually does with that table, so the
cells carry the same MathML the formula does, at `display="inline"`. A symbol the grammar
declines is escaped and printed as text, by the same rule the formula lines follow: a legend
