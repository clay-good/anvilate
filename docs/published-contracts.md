...
| [`docs/api/schemas/step-interface-candidates.schema.json`](api/schemas/step-interface-candidates.schema.json) | planar mating faces and regular through-hole patterns measured from imported STEP solids, with identity on multi-solid inputs | `INTERFACE_CANDIDATES_SCHEMA_VERSION` |
