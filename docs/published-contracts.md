...
| [`docs/api/schemas/step-interface-candidates.schema.json`](api/schemas/step-interface-candidates.schema.json) | planar faces, solid summaries, exact coplanar contacts, coaxial bore/shaft candidates, and regular through-hole patterns measured from imported STEP solids | `INTERFACE_CANDIDATES_SCHEMA_VERSION` |
