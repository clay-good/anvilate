...
**The required safety factor comes from the document.** Thirteen of the twenty-nine screens
...
| `assembly` | the build order from the declared parts, one access entry per adjustment in the state it is made in, and an inspectability summary with one entry per inspected dimension. A revision that moves an operation to another state changes the card and shows as that one line in the diff |
| `carbon` | one `embodied carbon` entry: the EN 15978 cradle-to-gate estimate over the declared lines, each with its factor's source, judged against the declared budget. With no budget the estimate is stated and the entry is not evaluated, and the needs report names the missing budget |
| `keepouts` | one `keepout <tag>` entry per declared keepout, not evaluated: intrusion is measured against the built part's geometry, which screening a document does not generate. The entry names the reason, the rule, the margin and the anchor, and says when the document no longer tags the anchor. A card with a declared keepout cannot pass on a volume nobody measured. On built geometry, `anvilate.keepouts.screen_keepouts` places each body on its anchor face and measures the part against it: material in the core fails with its volume, its depth along the keepout axis and a repair hint; material in the clearance margin fails as a margin intrusion; a clean keepout states its clearance; and a summary states how many were screened and the smallest clearance, at nominal geometry. The bodies export apart from the part: the keepout STEP writer puts them in a file of their own, each named KEEPOUT, its tag, non-manufacturing, and its reason, and the plate drawing puts each footprint on a KEEPOUT_NON_MANUFACTURING layer, so the machinable solid and its cut layers never carry one. Each generated rule is an archetype in the pattern library (keepout_prism/1, keepout_cylinder/1, keepout_frustum/1, keepout_swept_profile/1) under the same contribution contract as a part pattern: parameters, bounds, no DFM profile because it is never made, the screen that checks it, and five golden volumes each. [`examples/enclosure_keepouts.py`](../examples/enclosure_keepouts.py) thickens an enclosure floor into its service corridor and repairs it from the hint |
| `constraint_topology` | one `constraint topology` entry: each of the six freedoms counted against the declared constraints in the declared frame — FAIL naming a free freedom physically, or every constraint competing for an over-constrained one. When the load path is indeterminate, every element check that ran says so in its own line. See [constraint topology](constraint-topology.md) |
| `environment`, and each interface's kind and mating material | the [failure modes](failure-mode-coverage.md) that apply to the joint and the setting — galvanic corrosion across a dissimilar pair, fretting at a clamped or bolted face, thermal ratcheting under cycling — stated beside the verdict with what addressed each. Dissimilarity is derived from the two material references, never ticked by the author |
...
is exactly what a screening card cannot see.

`envelope` gets the same treatment and one extra piece of care. A `base_plate` declares its
bounding box outright, so the comparison is available — but **only what is true in every
orientation is a failure**. Sorting both triples and comparing pairwise answers whether the
solid fits *axis-aligned*, and that is a different question: a 300 × 300 × 25 mm plate misses
a 250 × 250 × 400 mm box on every axis and slides in diagonally. Calling that a `FAIL` is a
verdict against a part that fits — the mirror of a silent green, and just as wrong. So the
axis-aligned result is stated as evidence, and two conditions are failures because a rigid
