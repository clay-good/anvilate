...
- **It is not a promise of coverage.** Every one of the 1,820 public analysis symbols now
...
- **And two of them still name it only by accident.** The gate matches a curated list of
  sources as substrings, and an eponym is both a person and a quantity: a docstring that says
  "Poisson's ratio ν" or "the Reynolds number" satisfies it while naming no source at all.
  `Young` was struck from the list once for exactly this ("Young's modulus"); sweeping for the
  *shape* rather than the token found 50 more. Forty-eight are paid off, and the last two are
  enumerated in
