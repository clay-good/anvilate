...
class QuantityProperty(_Base):
    """A dimensional material property: a :class:`Quantity` plus its citation."""
...
class ScalarProperty(_Base):
    """A dimensionless material property (e.g. Poisson's ratio) plus its citation."""

    value: float
    citation: PropertyCitation


def _dimensioned(expected: str, name: str) -> AfterValidator:
    """Validator pinning the dimension of a :class:`QuantityProperty`'s quantity."""
...
Modulus = Annotated[QuantityProperty, _dimensioned("[pressure]", "elastic_modulus")]
Strength = Annotated[QuantityProperty, _dimensioned("[pressure]", "strength")]
Density = Annotated[QuantityProperty, _dimensioned("[mass] / [length] ** 3", "density")]
