...
from ..loads import CombinationSet, LoadNature, asce7_asd_basic, asce7_lrfd_basic
...
# 1.1.0 added the optional LoadCase.nature classification and the DesignSpec
# combination_basis (both additive; a 1.0.0 spec loads unchanged and is re-stamped).
...
class DesignSpec(_Base):
    ...
    # any. ``None`` leaves the spec evaluated per load case, as before.
    combination_basis: Literal["asce7_lrfd", "asce7_asd"] | None = None
    ...
    def combination_set(self) -> CombinationSet | None:
        ...
        §2.4.1 allowable-stress (ASD) basic set — so the spec-driven flow is
        ``spec.combination_set().governing(spec.combination_loads())``. The seismic
        sets, which need S_DS and the redundancy factor as inputs, are declared and
        generated by the caller rather than from the spec basis alone.
        """
