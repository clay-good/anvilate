...
def c_rate(*, current: Quantity, capacity: Quantity) -> float:
    ...
    hour, 0.5C in two, 2C in half an hour — it is how battery currents are quoted independent of pack
    size, and it sets the heating and voltage sag a cell sees. (The *actual* runtime falls short of
    the ideal 1/C-rate hours at high rates; the Peukert relations of
...
def current_from_c_rate(*, c_rate: float, capacity: Quantity) -> Quantity:
    ...
    The charge or discharge current a chosen ``c_rate`` draws from a pack of amp-hour ``capacity`` C:
    I = C-rate·C — the inverse of :func:`c_rate`. A 100 Ah pack at 0.5C delivers 50 A; at 2C it must
    push 200 A, which is what sizes the cabling, contactors, and BMS current limit. Returns the
    current in amperes.
    """
    _check(capacity, "[current]*[time]", "capacity")
    cap = capacity.to("A*hour").magnitude
...
def discharge_time_from_c_rate(*, c_rate: float) -> Quantity:
    ...
    The time a pack lasts at a steady ``c_rate``, on the nameplate assumption that usable capacity is
    rate-independent: t = 1/C-rate hours. A 0.5C draw runs two hours, a 2C draw half an hour. It is
    the first-cut runtime a designer quotes; at high rates the deliverable capacity shrinks (Peukert
    effect, :func:`anvilate.analysis.battery_peukert.peukert_runtime`), so treat this as an optimistic
    upper bound. Returns the discharge time in hours.
    """
    if c_rate <= 0:
