...
without a jump; it pays with a higher peak acceleration in the middle. Velocity
and acceleration follow by differentiating with respect to time, which brings down
one and two factors of the cam speed ω (v = ẏ = (dy/dθ)·ω, a = (d²y/dθ²)·ω²).
...
def cam_follower_motion(
    *,
    ...
    velocity but finite end acceleration) or ``"cycloidal"`` (acceleration zero at
    both ends, smoother but higher peak). ``rise`` L is the total lift, ``cam_angle``
