...
utilization**. The six rungs are the card's own roll-up order — `fail`, then
`not_evaluated`, then `warning`, then `over_margin`, then `out_of_depth`, then `pass` — so a
failing check outranks one that could not run, which outranks one inside a caution band its
document declared, which outranks an over-engineered one, which outranks a
...
Not every screen offers one: 30 of the library's 82 screens do, and
