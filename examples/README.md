...
| `bracket_bolt_group_eccentric.py` | A bracket hanging 40 kN 150 mm off its four-bolt group: sharing the load equally gives 10 kN/bolt (SF 2.50) but the eccentric moment drives the corner bolt to 27.6 kN by the elastic method (0.90, fails) — an eccentric bolt group is a torsion problem, and the moment can dwarf the direct shear. |
...
| `rotor_unbalance_response.py` | A fan rotor whose 0.02 mm residual unbalance is fine well below the critical speed (r 0.5, 0.03 mm) and above it (r 2.0, 0.007 mm) but spikes to 0.15 mm just under it (r 0.95) where the dynamic magnification factor hits 7.4 — a trim rotor shakes itself apart near resonance, so keep the running speed clear of the critical. |
...
| `cam_base_circle_pressure_angle.py` | A translating cam follower whose mid-rise pressure angle hits 31° on a tight 40 mm base circle (over the 30° jamming limit, SF 0.97) but drops to 23° when the base circle is opened to 60 mm (1.29) — a follower that binds is cured by growing the base circle, not by reprofiling the same lift. |
| `drivetrain_torsional_mode.py` | An engine-plus-load drivetrain whose two-rotor torsional mode sits at 35.6 Hz on a soft coupling — 0.89 of the 40 Hz firing frequency, in the danger band (SF 0.64) — but lifts to 79.6 Hz (1.99) on a stiff one; the mode depends on *both* inertias, and here the stiffer coupling is the safe one, carrying the mode up over the excitation. |
...
| `transmission_line_clearance.py` | A 400 m power span whose midspan sag must stay under 61 m: the parabolic approximation gives 60.0 m and passes, but the exact catenary sags 61.8 m and fails — the 3% the parabola shaves off is the difference between clearing and not, and it always under-predicts sag (the unsafe way) on a deep span. |
...
| `thin_tube_shell_buckling.py` | A Ø200 × 0.5 mm tube strut that looks bombproof as a column (Euler stress 2455 MPa, SF 12.3 at 200 MPa working) but crinkles locally: the shell-buckling stress knocked down to 182 MPa fails (0.91) — a thin-walled tube is governed by local wall buckling, not slenderness, so sizing it as a solid column overstates its strength 13×. |
...
| `steam_pipe_thermal_gradient.py` | A steam pipe whose 2 MPa pressure hoop stress is a trivial 20 MPa (SF 12.5) but whose through-wall thermal gradient — hot inside, cold outside, restrained — reaches 257 MPa (0.97, past yield); the stress that governs the pipe is not the one it was named for, which is why power-plant piping cracks by thermal fatigue at startups, not from pressure. |
