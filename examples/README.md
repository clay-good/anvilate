...
| `flat_bar_strut_weak_axis.py` | A 20 × 60 mm flat-bar strut screened about both axes: as drawn it is a stocky Johnson column at SF 4.6, but the weak axis (`least_radius_of_gyration`, r 5.8 vs 17.3 mm) is a slender Euler column and the same load fails at 1.6. |
...
| `stiffener_weld_end.py` | A propped barrier stiffener under hydrostatic triangle, welded at the sill vs at the top waler (`triangle_peak_at_prop`): peak-at-wall is stiff but overstressed, mirroring the fixity trims the wall moment 12.5% yet grows deflection 28% — the two weld ends fail opposite criteria. |
