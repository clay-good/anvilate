...
hand-verified, 5,449 tests), ISO 286 fits, tolerance stack-ups, DFM process-capability
...
| | |
| --- | --- |
| [`anvilate` on the command line](docs/headless-cli.md) | `build`, `check`, `export` and `diff`. Every command's `--help` includes a copyable example. `check` compiles a spec document, screens it and prints the card; the exit code follows the scorecard's own tri-state rather than collapsing to pass/fail. A document is bounded on every axis it has before any of it is screened, rendered, exported or signed — no infinity or NaN, no more than 32 levels of nesting, no string past 4,096 characters and no collection past 1,024 items — each refused at the front door naming the field. The three size bounds hold for a **scorecard** read back too — out of a signed attestation or a subject store — and for every model one holds. |
