...
Over 400 runnable examples, each executed in CI so they stay honest. A few:
...
| `pressure_vessel_nozzle_and_flange.py` | The same 800 mm vessel at two wall thicknesses: at 14 mm everything passes, and at 8 mm the shell still passes (SF 1.11) while the 6-inch opening fails at 0.49 — UG-37 credits the wall's *excess*, which falls away 3.4x faster than the wall itself. Its flange shows the second trap: the seating load is larger (400.0 vs 218.7 kN) and the operating condition still governs once each is divided by its own allowable, so the one-allowable shortcut lands 36% short. See [pressure equipment](docs/pressure-equipment.md). |
