...
hand-verified, 5,427 tests), ISO 286 fits, tolerance stack-ups, DFM process-capability
...
| [MCP server](docs/agent-mcp-integration.md) | All of the pipeline's eight operations over stdio, as `anvilate-mcp` or `python -m anvilate.mcp`; closed-form checks reply synchronously, while T3 uses durable task handles with structured progress, serialized state transitions, and subprocess cancellation. |
