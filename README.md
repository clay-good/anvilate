...
hand-verified, 5,426 tests), ISO 286 fits, tolerance stack-ups, DFM process-capability
...
| [MCP server](docs/agent-mcp-integration.md) | All of the pipeline's eight operations over stdio, as `anvilate-mcp` or `python -m anvilate.mcp`; closed-form checks reply synchronously, while T3 uses durable task handles with polling, serialized state transitions, and subprocess cancellation. |
