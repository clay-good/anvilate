...
| `lifting_lug_calc_report.py` | The same padeye screening rendered as a submittal, written as text, HTML and PDF: formula, substituted values, result, and clause for every check. See [calculation reports](docs/calculation-reports.md). |
...
(237 closed-form modules and 1,883 public symbols, each dimension-checked and
hand-verified, 6,504 tests), ISO 286 fits, tolerance stack-ups, DFM process-capability
...
| [Calculation reports](docs/calculation-reports.md) | Every check renders as a reviewable document: the formula, the values put into it, the result, and the clause. Units are chosen to *compose* rather than to look familiar. `to_pdf()` prints it for a submittal: byte-identical on reissue, no section split across a page, revision and page number on every page, and no font file embedded. |
| [What a citation means](docs/citations.md) | What a clause reference does and does not claim — and every one of the 1,883 public analysis symbols names one. Sixteen of the twenty-five bundled materials carry a specification minimum or an A- or B-basis allowable and screen unchanged; the other nine report `not_evaluated` until the caller declares that this screen accepts a typical value — a declaration that then lands on every entry the screen produced, including the passing ones. Eight of the sixteen are the [MIL-HDBK-5J allowables pack](docs/citations.md), which names its handbook table and says the handbook is superseded on every entry it produces. A [fatigue record](docs/citations.md) carries its curve, its survival level, what it was measured on, and where it came from, and cannot be built without all four. Data the library may read but may not ship goes through [fetch-on-first-use](docs/citations.md), where consent is an argument rather than a default. |
