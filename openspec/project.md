...
| Optical material data (roadmap) | refractiveindex.info database (CC0) + user-supplied or fetch-on-first-use vendor glass catalogs | CC0 / vendor terms | n(λ), Abbe, dn/dT, CTE, Knoop, stress-optic coefficient. Vendor catalogs are never redistributed — same doctrine as the AISC shapes DB |
| Optical design (interop only) | rayoptics (BSD-3), Optiland | BSD-3 / OSS | Not a dependency. Prescription and surface-deformation quantities are typed imports with tool identity and version recorded; Anvilate does not ray trace |
...
**Version pinning (verified 2026-08-22):** OCCT 7.9.x via OCP 7.9.3.1.1, and the pin stands. OCCT itself is at 8.0.1 upstream, but **no OCP 8.x has been published to PyPI** (`cadquery-ocp` tops out at 7.9.3.1.1) and build123d 0.11.1 requires `cadquery-ocp-novtk >=7.9,<8.0`, so there is nothing to migrate to yet. **Bump condition, recorded so nobody re-litigates it:** an OCP 8.x release on PyPI *and* build123d relaxing its `<8.0` cap — check both before spending time on the migration. build123d 0.11.1, CadQuery 2.8.0, Gmsh 4.15.2 (note: 4.15 changed `Boundary` tag signedness), CalculiX ccx 2.23 (2.24 expected on the usual late-summer cadence — golden regressions must gate the bump), ezdxf 1.4.4. CalculiX buckling results are version-sensitive — every physics gate carries golden-result regression cases against the pinned solver version. pycalculix is abandoned: write `.inp` decks directly and parse `.frd`/`.dat` in-repo (ccx2paraview for VTK). Avoid deep coupling to meshio (unmaintained).
