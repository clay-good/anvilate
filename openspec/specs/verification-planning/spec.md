...
## Purpose
The physical test each passing check implies, derived from the clause the check already cites: a lifting device's proof load, a vessel's hydrostatic test, a toleranced feature's inspection. The plan carries the acceptance criterion the standard sets and the instrument accuracy it needs, states honestly which checks it cannot cover, and is never evidence — nothing performed reports not-evaluated, and only a recorded result turns a plan into proof.
