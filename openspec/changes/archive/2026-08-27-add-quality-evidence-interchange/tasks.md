...
- [x] 1.2 Serializer validating against the QIF schemas; unevaluated-characteristic
      ...
      and no actual. Schema validation is opt-in locally (`ANVILATE_QIF_XSD` + `lxml`),
      because the schema package is a separate free download and the parser is not a runtime
      dependency, and the test skips rather than passing when either is absent. CI's
      `interchange-schemas` job fetches both packages and runs it for real weekly, failing if
      it skipped
