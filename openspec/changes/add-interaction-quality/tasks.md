...
- [ ] 2.1 Remedy field on every refusal: the action, its concrete subject, and where a
      ...
      structured field is `ScorecardEntry.needs` (add-declaration-completeness): 36 of the
      library's 111 not-evaluated entries carry it (nine in screening, every optomechanics
      and assembly screen that stops for a value, four pressure-design screens and two callout entries), and tests/test_needs.py now sweeps every module, not
